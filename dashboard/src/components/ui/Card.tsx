...
export function KpiCard({ title, value, icon: Icon, delta, deltaType = 'neutral', isLoading, description, onClick }: KpiCardProps) {
  return (
    <Card className="group relative min-h-[124px] overflow-hidden" title={undefined} onClick={onClick}>
      ...
      <div className="relative flex min-h-[68px] flex-col justify-between gap-3 pr-10">
      ...
      {delta && !isLoading && (
        <div className="relative mt-3 flex items-center border-t border-[var(--card-border)] pt-3">
          <span
            className={cn(
