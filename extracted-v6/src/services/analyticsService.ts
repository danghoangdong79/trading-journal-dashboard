...
export class AnalyticsService {
  ...
  static summarizeTrades(trades: Trade[]): TradeSummary {
    ...
    const winning = closedTrades.filter((trade) => trade.status === 'Th\u1eafng' || trade.netPnL > 0);
    const losing = closedTrades.filter((trade) => trade.status === 'Thua' || trade.netPnL < 0);
    const breakEven = closedTrades.filter((trade) => trade.status === 'H\u00f2a' || trade.netPnL === 0);
