...
try {
  // Use the safest way to get Global Config in modern n8n
  const sheetId = $('Global Config').first().json.SHEET_ID;
  debugLog.push('sheetId=' + sheetId);
  ...
  if (apiRes && apiRes.trades) {
    ...
    debugLog.push('activeTrades=' + activeTrades.length);
  }
} catch(e) {
  debugLog.push('FetchError: ' + e.message);
}
...
for (let i = 0; i < parsedArr.length; i++) {
  ...
  if (draft.symbol && draft.position) {
    ...
    // Find active trade for this symbol AND account
    ...
    if (existingTrade) {
      debugLog.push('Found existing trade for ' + sym);
      ...
      debugLog.push('No active trade for ' + sym);
  ...
  // Attach debug info to the first trade just to trace issues easily
  if (i === 0 && debugLog.length > 0) {
