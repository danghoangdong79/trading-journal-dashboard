const APP_NAME = 'Dahodo Journal React Clone';
...
function getDatasetForFetch(sheetId, forceRefresh) {
  return getDataset_({ sheetId: sheetId, forceRefresh: Boolean(forceRefresh) });
}
...
function getDataset_(options) {
  ...
  const journalValues = readRangeSafe_(spreadsheet, SHEET_RANGE);
  const cashFlowValues = readRangeSafe_(spreadsheet, CASHFLOW_RANGE);
  const feeValues = readRangeSafe_(spreadsheet, FEE_CHARGES_RANGE);
  const configValues = readRangeSafe_(spreadsheet, CONFIG_RISK_RANGE);
  const userValues = readRangeSafe_(spreadsheet, USERS_RANGE);
...
function readRangeSafe_(spreadsheet, a1Range) {
  try {
...
function parseDateTime_(dateStr, timeStr) {
  ...
  const timeValue = normalizeTime_(timeStr) || '00:00:00';
  const slash = dateValue.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (slash) {
    return validDate_(new Date(slash[3] + '-' + pad2_(slash[2]) + '-' + pad2_(slash[1]) + 'T' + timeValue));
  }
  const iso = dateValue.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
  if (iso) {
    return validDate_(new Date(iso[1] + '-' + pad2_(iso[2]) + '-' + pad2_(iso[3]) + 'T' + timeValue));
  }
  const parsed = new Date(dateValue + ' ' + timeValue);
  return validDate_(parsed);
}
...
function normalizeTime_(value) {
  ...
  return pad2_(hours) + ':' + pad2_(minutes) + ':' + pad2_(seconds);
}
...
function toIsoString_(date) {
  return validDate_(date) ? date.toISOString() : null;
}
...
function normalizeStatus_(value) {
  ...
  if (normalized === 'thang') return 'Tháº¯ng';
  if (normalized === 'thua') return 'Thua';
  if (normalized === 'hoa') return 'HÃ²a';
  if (normalized === 'dang mo') return 'Äang má»Ÿ';
  return 'Äang má»Ÿ';
}
  ...
  return (values || []).slice(1)
    ...
    .map(function(row, index) {
      return {
        ...
        category: String(row[4] || 'PhÃ­ Ä‘á»‹nh ká»³').trim(),
        amount: Math.abs(parseNumber_(row[11] || row[10] || row[9])),
        note: [String(row[3] || '').trim(), String(row[1] || '').trim()].filter(String).join(' Â· ')
      };
    })
...
function getAvailableAccounts_(spreadsheet) {
  for (var i = 0; i < ACCOUNT_LIST_RANGES.length; i += 1) {
    const values = readRangeSafe_(spreadsheet, ACCOUNT_LIST_RANGES[i]);
...
function mapRows_(values, cashFlows, initialCapital) {
  ...
  const trades = (values || []).slice(1)
    .filter(function(row) { return row && row.some(function(cell) { return String(cell || '').trim() !== ''; }); })
    .map(function(row, index) {
      const netPnL = parseNumber_(row[20]);
      ...
      return {
        ...
        assetType: normalizeAssetType_(row[2]),
        symbol: String(row[3] || '').trim(),
        position: normalizePosition_(row[4]),
        ...
        closeDate: closeDate,
        closeTime: closeTime,
        holdingDays: parseNumber_(row[11]),
        volume: parseNumber_(row[12]),
        entryPrice: parseNumber_(row[13]),
        exitPrice: parseNumber_(row[14]),
        stopLoss: parseNumber_(row[15]),
        takeProfit: parseNumber_(row[16]),
        amplitude: parseNumber_(row[17]),
        grossPnL: parseNumber_(row[18]),
        feesAndTaxes: parseNumber_(row[19]),
        netPnL: netPnL,
        mood: String(row[21] || '').trim(),
        reviewNote: String(row[22] || '').trim(),
        sector: String(row[23] || '').trim(),
        entryDateTime: toIsoString_(parseDateTime_(openDate, openTime)),
        exitDateTime: toIsoString_(parseDateTime_(closeDate, closeTime)),
        cashFlow: cashFlow,
